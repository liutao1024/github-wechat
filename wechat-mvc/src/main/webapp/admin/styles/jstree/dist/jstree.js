	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, undefined) {
			...
			var instance_counter = 0, ccp_node = false, ccp_mode = false, ccp_inst = false, themes_loaded = [], src = $(
					'script:last').attr('src'), _d = document, _node = _d
			 ...
			 * with an ID of "branch" // all of there will return the same
			 * instance $.jstree.reference('tree'); $.jstree.reference('#tree');
			...
			$.jstree.defaults.core = {
				 ...
				 * 
				 * __Examples__
				 *  // AJAX $('#tree').jstree({ 'core' : { 'data' : { 'url' :
				 * '/get/children/', 'data' : function (node) { return { 'id' :
				 * node.id }; } } });
				 *  // direct data $('#tree').jstree({ 'core' : { 'data' : [
				 * 'Simple root node', { 'id' : 'node_2', 'text' : 'Root node
				 * with options', 'state' : { 'opened' : true, 'selected' : true },
				 * 'children' : [ { 'text' : 'Child 1' }, 'Child 2'] } ] });
				 *  // function $('#tree').jstree({ 'core' : { 'data' : function
				 * (obj, callback) { callback.call(this, ['Root 1', 'Root 2']); }
				 * });
				 * 
			...
			$.jstree.core.prototype = {
				...
				init : function(el, options) {
					...
					this.element
							.bind("destroyed", $.proxy(this.teardown, this));
				...
				get_node : function(obj, as_dom) {
					...
					try {
						...
						} else if (((dom = $(obj, this.element)).length || (dom = $(
								'#' + obj.replace($.jstree.idregex, '\\$&'),
								this.element)).length)
								&& this._model.data[dom.closest('.jstree-node')
										.attr('id')]) {
							obj = this._model.data[dom.closest('.jstree-node')
									.attr('id')];
				...
				get_next_dom : function(obj, strict) {
					...
					return obj.parentsUntil(".jstree", ".jstree-node").next(
							".jstree-node:visible").eq(0);
				},
				/**
				...
				get_prev_dom : function(obj, strict) {
					...
					if (tmp !== null) {
						...
						while (obj.hasClass("jstree-open")) {
							obj = obj.children(".jstree-children:eq(0)")
				...
				load_node : function(obj, callback) {
					...
					this._load_node(obj, $.proxy(function(status) {
						...
						if (obj.state.loaded && !obj.children.length && dom
								&& dom.length && !dom.hasClass('jstree-leaf')) {
							dom.removeClass('jstree-closed jstree-open')
				...
				_load_nodes : function(nodes, callback, is_callback) {
					...
					for (i = 0, j = nodes.length; i < j; i++) {
						if (m[nodes[i]]
								&& (!m[nodes[i]].state.loaded || !is_callback)) {
				...
				_load_node : function(obj, callback) {
					...
					if (typeof s === 'string') {
						if (obj.id === '#') {
							return this._append_html_data(obj, $(s), function(
									status) {
								callback.call(this, status);
							});
						} else {
				...
				_parse_model_from_html : function(d, p, ps) {
					...
					tmp = d.children("a:eq(0)").length ? d.children("a:eq(0)")
							.clone() : d.clone();
				 ...
				 */
				_redraw : function() {
					var nodes = this._model.force_full_redraw ? this._model.data['#'].children
							.concat([])
				...
				open_all : function(obj, animation, original_obj) {
					...
					var dom = obj.id === '#' ? this.get_container_ul() : this
							.get_node(obj, true), i, j, _this;
					if (!dom.length) {
				...
				close_all : function(obj, animation) {
					...
					var dom = obj.id === '#' ? this.get_container_ul() : this
							.get_node(obj, true), _this = this, i, j;
					if (!dom.length) {
						for (i = 0, j = obj.children_d.length; i < j; i++) {
				...
				dehover_node : function(obj) {
					...
					obj.attr('aria-selected', false).children('.jstree-anchor')
				...
				select_all : function(supress_event) {
					...
					this._data.core.selected = this._model.data['#'].children_d
							.concat();
				...
				set_state : function(state, callback) {
					if (state) {
						if (state.core) {
							 ...
							 * this[ state.core.themes.dots ? "show_dots" :
							 * "hide_dots" ](); } if(typeof
							 * state.core.themes.icons !== 'undefined') { this[
							 * state.core.themes.icons ? "show_icons" :
							 * "hide_icons" ](); } delete state.core.themes;
							 * delete state.core.open; this.set_state(state,
				 ...
				 * @param {Boolean}
				 *            internal parameter indicating if the parent node
				 *            has been loaded
				...
				move_node : function(obj, par, pos, callback, is_loaded) {
					...
					if ($.isArray(obj)) {
						...
						for (t1 = 0, t2 = obj.length; t1 < t2; t1++) {
							this.move_node(obj[t1], par, pos, callback,
									is_loaded);
						}
						return true;
					}
					...
					new_par = (!pos.toString().match(/^(before|after)$/) || par.id === '#') ? par
				...
				copy_node : function(obj, par, pos, callback, is_loaded) {
					...
					if ($.isArray(obj)) {
						obj = obj.reverse().slice();
						for (t1 = 0, t2 = obj.length; t1 < t2; t1++) {
							this.copy_node(obj[t1], par, pos, callback,
									is_loaded);
						}
						return true;
					}
				 ...
				 *            (if omitted the node text value is used)
				 */
				edit : function(obj, default_text) {
					...
					var rtl = this._data.core.rtl, w = this.element.width(), a = obj
							.children('.jstree-anchor'), s = $('<span>'),
					/*
					 * ! oi = obj.children("i:visible"), ai =
					 * a.children("i:visible"), w1 = oi.width() * oi.length, w2 =
					 * ai.width() * ai.length,
					 */
					t = default_text, h1 = $("<" + "div />", {
						css : {
							"position" : "absolute",
							"top" : "-200px",
							"left" : (rtl ? "0px" : "-1000px"),
													...
													if (this
															.rename_node(
																	...
																	$(
																			'<div></div>')
																			.text(
																					v)[this.settings.core.force_text ? 'text'
																			: 'html']
																			()) === false) {
				...
				set_icon : function(obj, icon) {
					...
					if (icon === false) {
						...
						dom.removeClass('jstree-themeicon-custom ' + old).css(
								"background", "").removeAttr("rel");
			...
			$.jstree.plugins.checkbox = function(options, parent) {
												...
												.proxy(
														function(e, data) {
															...
															for (
																	i = 0,
																	j = dpc.length; i < j; i++) {
																m[dpc[i]].state.checked = (m[dpc[i]].original
								...
								.on(
										...
										$
												.proxy(
														function(e, data) {
															var obj = this
																	.get_node('#'), m = this._model.data, i, j, tmp;
															for (
																	i = 0,
																	j = obj.children_d.length; i < j; i++) {
																tmp = m[obj.children_d[i]];
				...
				this._undetermined = function() {
					var i, j, m = this._model.data, t = this.settings.checkbox.tie_selection, s = this._data[t ? 'core'
				...
				this.check_all = function() {
					...
					this._data.checkbox.selected = this._model.data['#'].children_d
							.concat();
			...
			$.jstree.defaults.contextmenu = {
				 ...
				 * be a separator before this item * `separator_after` - a
				 * boolean indicating if there should be a separator after this
				 * item * `_disabled` - a boolean indicating if this action
				 * should be disabled * `label` - a string - the name of the
				 * action (could be a function returning a string) * `action` -
				 * a function to be executed if this item is chosen * `icon` - a
				 * string, can be a path to an icon or a className, if using an
				 * image that is in the current directory use a `./` prefix,
				 * otherwise it will be detected as a class * `shortcut` -
			...
			$.jstree.defaults.dnd = {
				 ...
				 * be dragged as an argument (array) - return `false` to prevent
				 * dragging
				 * 
			...
			$(function() {
				...
				var lastmv = false, laster = false, opento = false, marker = $(
				...
				$(document)
						...
						.on(
								...
								function(e, data) {
									...
									var ins = $.jstree
											.reference(data.event.target), ref = false, off = false, rel = false, l, t, h, p, i, o, ok, t1, t2, op, ps, pr, ip, tm;
						...
						.on(
								...
								function(e, data) {
									...
									marker.hide();
									lastmv = false;
									data.helper.find('.jstree-icon:eq(0)')
											.removeClass('jstree-ok').addClass(
													'jstree-er');
								})
						.on(
			...
			(function($) {
				...
				$.vakata.search = function(pattern, txt, options) {
					...
					pattern = options.caseSensitive ? pattern : pattern
							.toLowerCase();
					var MATCH_LOCATION = options.location || 0, MATCH_DISTANCE = options.distance || 100, MATCH_THRESHOLD = options.threshold || 0.6, patternLen = pattern.length, matchmask, pattern_alphabet, match_bitapScore, search;
					if (patternLen > 32) {
						options.fuzzy = false;
					}
					if (options.fuzzy) {
			...
			$.jstree.plugins.types = function(options, parent) {
				...
				this.set_type = function(obj, type) {
					...
					obj.type = type;
					if (old_icon === true
							|| (t[old_type] && t[old_type].icon && old_icon === t[old_type].icon)) {
						this.set_icon(obj,
