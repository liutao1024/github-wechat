...
$.extend($.validator, {
	...
	prototype: {
		...
		showLabel: function(element, message) {
			if (!!message && !this.errorList.length) {
				$("._msgtips_").show();
			}
			...
			if ( label.length ) {
				...
				var msgLenght = $.trim(message).replace(/[^\x00-\xff]/g,"**").length;
				label.css("width",(msgLenght*6.8)+"px");
				label.attr("generated") && label.html(message);
				label.prepend("<div class='tooltip-before'>&nbsp;&nbsp;</div>");
			} else {
				var msgLenght = $.trim(message).replace(/[^\x00-\xff]/g,"**").length;
				// create label
				label = $("<" + this.settings.errorElement + " onclick='$(this).hide()'/>")
					.attr({"for":  this.idOrName(element), generated: true})
					.addClass(this.settings.errorClass).addClass("_msgtips_").css("width",(msgLenght*6.8)+"px")
					.html(message || "");
				label.prepend("<div class='tooltip-before'>&nbsp;&nbsp;</div>");
				if ( this.settings.wrapper ) {
...
;(function($) {
	...
	//设置jquery validate的默认样式
	$.validator.setDefaults({
		wrapper:"div class='element-relative'",
